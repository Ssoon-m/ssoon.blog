...
[이전 글에서 작성한 AlertDialog](https://ianlog.me/blog/alert-dialog-atoz)와 Presence컴포넌트를 적용한 `ContentImpl` 컴포넌트에 `FocusTrap` 컴포넌트를 사용하는 예시로 설명을 하겠습니다.
...
### 적용 전
![no-focus-trap](/posts/blog/alert-dialog-focus-trap/no-focus-trap.gif)
...
AlertDialog에 FocusTrap을 제대로 적용시키려면 ContentImpl컴포넌트에 다음과 같이 콜백 함수를 만들어주면 됩니다. 
...
```
위와 같은 과정을 다 마쳤다면 FocusTrap컴포넌트를 AlertDialog에서 정상적으로 사용이 가능합니다.
