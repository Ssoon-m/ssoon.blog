---
title: 'Vanilla JS로 함수형 리액트 만들기(react hooks)'
date: 2024-03-10
...
```ts {51-63} title="src/lib/dom/index.ts"
...
```ts title="src/lib/dom/utils/object.ts"
...
```ts {4} title="src/lib/dom/index.ts"
const useState = <T>(initialState?: T) => {
    //...
    const setState = (newState: T) => {
...
```ts {6,8} title="src/lib/dom/index.ts"
const useState = <T>(initialState?: T) => {
    //...
    const setState = (newState: T) => {
      if(shallowEqual(state,newState)) return;
...
```ts {1-7,12,20} title="src/lib/dom/index.ts"
...
```ts {4-6,16,19-20} title="src/lib/dom/index.ts"
  const resetOptions = () => {
    options.states = [];
    options.stateHook = 0;
    options.effectList = [];
